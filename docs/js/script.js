...
//一番上
(function(d){
  const top = d.querySelector(".header");
  const bellow = d.querySelector(".about");
  const o = new IntersectionObserver(function(es){
    if (es[0].intersectionRatio < 0.8) {
      top.classList.add("hide");
      bellow.classList.add("show");
    } else {
      top.classList.add("transit");
      top.classList.remove("hide");
      bellow.classList.remove("show");
    }
  },{
    threshold: [0, 0.7, 0.8, 0.9, 1],
  });
  if (window.pageYOffset < 300) {
    top.classList.remove("transit");
  }
  o.observe(top);
})(document);
...
(function(d){
  function scrollPage(btn) {
    btn.addEventListener("click", function(e){
      e.preventDefault();
      d.querySelector(this.hash).scrollIntoView({behavior:'smooth'});
    });
  }
  Array.prototype.forEach.call(d.getElementsByClassName("scroll-btn"), scrollPage);
})(document);
...
(function(d){
  ...
  const o = new IntersectionObserver(function(es){
    if (es[0].intersectionRatio === 0) {
      goTop.classList.add("show");
    } else {
      goTop.classList.remove("show");
    }
  });
  o.observe(d.querySelector(".page-top"));
})(document);
...
(function(d){
  ...
  const elems = d.getElementsByClassName("show-up");
